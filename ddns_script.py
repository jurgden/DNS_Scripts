...
def update_dns_record(zone_id, record_name, record_type, content, ttl, api_token):
    ...
    response = requests.get(list_url, headers=headers, params={"name": record_name, "type": record_type})
    ...
    if current_ip != content:
        ...
        data = {"type": record_type, "name": record_name, "content": content, "ttl": ttl}
...
try:
    ...
    update_dns_record(zone_id, record_name, record_type, current_ip, ttl, api_token)
except Exception as e:
    print(str(e))
